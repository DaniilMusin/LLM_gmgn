...
def get_recent_amm_pi(contract: str, minutes: int = 60) -> float | None:
    init_db(); conn=_conn()
    row = conn.execute("SELECT MIN(amm_pi_pct) AS min_pi FROM trades WHERE contract=? AND ts >= datetime('now', ?)", (contract, f'-{minutes} minutes')).fetchone()
    conn.close()
