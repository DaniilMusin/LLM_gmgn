...
# BUG FIX #17: Thread-safe lock for control.json operations
...
def get_dry_run() -> bool:
    with _CONTROL_LOCK:
        return bool(_load().get("dry_run", True))

def set_dry_run(v: bool):
    with _CONTROL_LOCK:
        d=_load(); d["dry_run"]=bool(v); _save(d)

def get_size_sol() -> float:
    with _CONTROL_LOCK:
        return float(_load().get("size_sol", _DEFAULT["size_sol"]))

def set_size_sol(v: float):
    with _CONTROL_LOCK:
        d=_load(); d["size_sol"]=float(v); _save(d)

def get_size_usdc() -> float:
    with _CONTROL_LOCK:
        return float(_load().get("size_usdc", _DEFAULT["size_usdc"]))

def set_size_usdc(v: float):
    with _CONTROL_LOCK:
        d=_load(); d["size_usdc"]=float(v); _save(d)
...
def is_source_enabled(name: str) -> bool:
    with _CONTROL_LOCK:
        return bool(_load().get("sources", {}).get(name, False))

def set_source_enabled(name: str, enabled: bool):
    with _CONTROL_LOCK:
        d=_load(); s=d.get("sources", {}); s[name]=bool(enabled); d["sources"]=s; _save(d)
